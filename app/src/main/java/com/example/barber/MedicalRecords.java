...
public class MedicalRecords extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            Toast.makeText(MedicalRecords.this, imageuri.toString(), Toast.LENGTH_SHORT).show();

            // Here we are uploading the pdf in firebase storage with the name of current time
            final StorageReference filepath = storageReference.child(messagePushID + "." + "pdf");
            Toast.makeText(MedicalRecords.this, filepath.getName(), Toast.LENGTH_SHORT).show();
            filepath.putFile(imageuri).continueWithTask(new Continuation() {
                ...
                public void onComplete(@NonNull Task<Uri> task) {
                    if (task.isSuccessful()) {
                        ...
                        dialog.dismiss();
                        Toast.makeText(MedicalRecords.this, "UploadedFailed", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
